...
order: IOrder | null = null; // даные заказа
...
получить итоговую сумму в корзине `getTotal, возвращает число `, валидирует форму заказа `validateOrder` и `setOrderField, принимает ключ из IOrder и устанавливает значение`,
устанавливает превью `setPreview, принимает еденичный товар`,
фильтрует корзину с товарами `toggleOrderedProducts, принимает еденичный товар`, `addToBasket, принимает еденичный товар` позволяет добавить продукт в корзину,
...
4. Класс `PreviewItem` наследуется от класса `Card`
