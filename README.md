...
1. Класс `AppData` имплементирует интерфейс `IAppState`, реализует единую модель данных приложения, отвечает за хранение данных и работу с ними. 
...
Позволяет установить информацию о каталоге продуктов `setCatalog, принимает список продуктов`, очистить корзину `clearBasket`, 
...
а `deleteFromBasket, принимает еденичный товар` удалить из корзины, `getBasketProducts` возвращает список продуктов из корзины.
...
```
В конструкторе принимает контейнер и объект брокера событий `container: HTMLElement,protected events: IEvents`
...
В конструкторе принимает контейнер и объект брокера событий `protected container: HTMLFormElement,protected events: IEvents`
...
Отвечает за закрытие и открытие модального окна `open` и `close`, устанавливает в него контент. Переопределяет метод `render` родительского класса.
...
Устанавливает текст итоговой суммы покупки.

`set total(value: number)`

### Компонент `Order.ts`

Отвечает за вывод ошибок и работу с компонентами двух форм заказа:
1) Форма с выбором способа оплаты и адресом `paymentOrder`
2) Форма с вводом e-mail и телефона `contactsOrder`
...
Наследуется от класса `Form` с параметром полиморфного типа `IOrderForm` и отвечает за установку полей формы заказа с адресом и выбором оплаты. Устанавливает слушатели на кнопки выбора типа оплаты и соответствующие стили. 
...
Отвечает за установку всех свойств карточки, используется для отображения всех видов карточек. 
...
4. `order:next` переход на следующую страница заказа
5. `modal:open` и `modal:close` вызываются при открытии и закрытии модальных окон
6. `card:select` выбрана карточка товара 
7. `preview:changed` вызывается при изменении выбора карточки
